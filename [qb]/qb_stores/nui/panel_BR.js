window.addEventListener('message', function (event) {
	...
	if (item.showmenu){
		...
		if(item.isMarket == true){
			...
			$('#nav-bar').append(`
				...
				<li onclick="closeUI()">
					...
					<span class="tooltip">Fechar</span>
				</li>
			`);
		}else{
			$('#nav-bar').append(`
				<li id="sidebar-1" onclick="openPage(0)">
					...
					<span class="tooltip">Sua loja</span>
				</li>
				<li onclick="openPage(1)">
					<i class="fas fa-pallet"></i>
					<span class="tooltip">Comprar produtos</span>
				</li>
				<li onclick="openPage(2)">
					<i class="fas fa-users"></i>
					<span class="tooltip">Contratar entregador</span>
				</li>
				...
				<li href="#myModal" data-toggle="modal" >
					<i class="fas fa-times"></i>
					<span class="tooltip">Vender loja</span>
				</li>
			`);
		}
		...
		$('#profile-stock-2').append('Capacidade do estoque (Max: ' + stock_capacity + ')');
					...
					<div class="d-flex flex-row align-items-center"><i class="fa fa-box checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 small">
								<div><i class="fas fa-coins"></i><span class="ml-2">Custo: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency }).format(price-discount) + `</span></div>
								<div class="ml-3"><i class="fas fa-boxes"></i><span class="ml-2">Quantidade: ` + amount + `</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
						<button onclick="startJob('` + key + `')" class="btn btn-blue btn-darken-2 white">Iniciar Trabalho</button>
					</div>
				</li>
			`);
							...
							<div class="card-body cleartfix">
								<div class="media align-items-stretch">
									...
									<div class="media-body">
										<h4>` + item.name + `</h4>
										<span> Preço: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency }).format(item.price_to_customer)  + `</span>
									</div>
									<div class="align-self-center">
						...
						<div class="card-content">
							<div class="card-body cleartfix">
								<div class="media align-items-stretch">
									...
									<div class="media-body">
										<h4>` + item.name + ` <small>Estoque (` + stock_value + `)</small> </h4>
										<input id="input-` + fkey + `" class="deposit-money pr-0 pl-0 col-6" type="number" min="1" max="` + arr_stock[key] + `" placeholder="Quantidade" name="amount" required="required">
										<button onclick="buyItem('` + fkey + `')" class="btn btn-blue btn-darken-2 white deposit-money-btn col-6">Comprar</button>
									</div>
									<div class="align-self-center">
				...
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><i class="fa fa-box checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 small">
								<div><i class="fas fa-coins"></i><span class="ml-2">Salário: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency }).format(store_job.reward) + `</span></div>
								<div class="ml-3"><i class="fas fa-coins"></i><span class="ml-2">Custo total: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency }).format(total_cost) + `</span></div>
								...
								<div class="ml-3"><i class="fas fa-boxes"></i><span class="ml-2">Quantidade: ` + store_job.amount + `</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
						<button onclick="deleteJob(` + store_job.id + `)" class="btn btn-red btn-accent-4 white">Deletar contrato</button>
					</div>
				</li>
			`);
		}
			...
			<div class="col-lg-4 col-md-6 mb-4 mb-lg-0">
				<div class="card rounded shadow-sm border-0">
					<div class="card-body p-4"><img src="https://cdn.discordapp.com/attachments/533398980428693550/804898333192880128/shop.png" alt="" class="img-fluid d-block mx-auto mb-3">
						<h5> <a href="#" class="text-dark">Capacidade de estoque</a></h5>
						<p style="height: 65px;" class="small text-muted font-italic">Melhore sua capacidade de estoque para obter mais espaço para armazenar seus produtos. Seus produtos serão vendidos apenas quando alguém vier à sua loja.</p>
						<ul class="small text-muted font-italic">
							<li> Level 1: +` + config.market_types.upgrades.stock.level_reward[1] + ` unidades.</li>
							<li> Level 2: +` + config.market_types.upgrades.stock.level_reward[2] + ` unidades. </li>
							<li> Level 3: +` + config.market_types.upgrades.stock.level_reward[3] + ` unidades. </li>
							<li> Level 4: +` + config.market_types.upgrades.stock.level_reward[4] + ` unidades. </li>
							<li> Level 5: +` + config.market_types.upgrades.stock.level_reward[5] + ` unidades. </li>
						</ul>
						<ul class="justify-content-center d-flex list-inline small">
							` + getStarsHTML(store_business.stock_upgrade) + `
						</ul>
						<button onclick="buyUpgrade('stock')" class="btn btn-blue btn-darken-2 white btn-block">Comprar ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(config.market_types.upgrades.stock.price) + `</button>
					</div>
				</div>
			</div>
			...
			<div class="col-lg-4 col-md-6 mb-4 mb-lg-0">
				<div class="card rounded shadow-sm border-0">
					<div class="card-body p-4"><img src="https://cdn.discordapp.com/attachments/533398980428693550/804898331159298078/delivery-truck.png" alt="" class="img-fluid d-block mx-auto mb-3">
						<h5> <a href="#" class="text-dark">Capacidade do caminhão</a></h5>
						<p style="height: 65px;" class="small text-muted font-italic">Melhore a capacidade do seu caminhão para obter um caminhão melhor para transportar uma maior quantidade de produtos. Você receberá este valor de bônus quando terminar a entrega.</p>
						<ul class="small text-muted font-italic">
							<li> Level 1: +` + config.market_types.upgrades.truck.level_reward[1] + ` unidades.</li>
							<li> Level 2: +` + config.market_types.upgrades.truck.level_reward[2] + ` unidades. </li>
							<li> Level 3: +` + config.market_types.upgrades.truck.level_reward[3] + ` unidades. </li>
							<li> Level 4: +` + config.market_types.upgrades.truck.level_reward[4] + ` unidades. </li>
							<li> Level 5: +` + config.market_types.upgrades.truck.level_reward[5] + ` unidades. </li>
						</ul>
						<ul class="justify-content-center d-flex list-inline small">
							` + getStarsHTML(store_business.truck_upgrade) + `
						</ul>
						<button onclick="buyUpgrade('truck')" class="btn btn-blue btn-darken-2 white btn-block">Comprar ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(config.market_types.upgrades.truck.price) + `</button>
					</div>
				</div>
			</div>
			...
			<div class="col-lg-4 col-md-6 mb-4 mb-lg-0">
				<div class="card rounded shadow-sm border-0">
					<div class="card-body p-4"><img src="https://cdn.discordapp.com/attachments/533398980428693550/804898332371189780/manager.png" alt="" class="img-fluid d-block mx-auto mb-3">
						<h5> <a href="#" class="text-dark">Relacionamento</a></h5>
						<p style="height: 65px;" class="small text-muted font-italic">Quanto melhor for o seu relacionamento com os fornecedores, melhores serão os preços de compra. Você receberá o desconto ao começar um novo trabalho.</p>
						<ul class="small text-muted font-italic">
							<li> Level 1: ` + config.market_types.upgrades.relationship.level_reward[1] + `% desconto. </li>
							<li> Level 2: ` + config.market_types.upgrades.relationship.level_reward[2] + `% desconto. </li>
							<li> Level 3: ` + config.market_types.upgrades.relationship.level_reward[3] + `% desconto. </li>
							<li> Level 4: ` + config.market_types.upgrades.relationship.level_reward[4] + `% desconto. </li>
							<li> Level 5: ` + config.market_types.upgrades.relationship.level_reward[5] + `% desconto. </li>
						</ul>
						<ul class="justify-content-center d-flex list-inline small">
							` + getStarsHTML(store_business.relationship_upgrade) + `
						</ul>
						<button onclick="buyUpgrade('relationship')" class="btn btn-blue btn-darken-2 white btn-block">Comprar ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(config.market_types.upgrades.relationship.price) + `</button>
					</div>
				</div>
			</div>
		`);
					...
					<li class="d-flex justify-content-between">
						<div class="d-flex flex-row align-items-center"><i class="fa fa-plus-circle checkicon"></i>
							<div class="ml-2">
								...
								<div class="d-flex flex-row mt-1 text-black-50 small">
									<div><i class="fas fa-coins"></i></i><span class="ml-2">Valor: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency }).format(balance.amount) + `</span></div>
									<div class="ml-3"><i class="fas fa-calendar-alt"></i></i><span class="ml-2">Data: ` + timeConverter(balance.date,config.format.location) + `</span></div>
								</div>
							</div>
						</div>
					</li>
...
function setMaxAmount(item_id,amount){
	$('#form_amount').empty();
	$('#form_amount').append(`
		<input type="number" min="1" max="` + amount + `" name="amount" class="form-control" placeholder="Quantidade" required="required"> 
	`);
}
